...
import org.openmrs.module.cflcore.api.converter.PatientFilterConverter;
...
import org.openmrs.module.cflcore.web.model.AdHocMessageChannelConfigurationBuilderFactory;
import org.openmrs.module.cflcore.web.model.AdHocMessageControllerModel;
...
public class AdHocMessageController {
  ...
  @ApiResponses(
      value = {
        @ApiResponse(
            ...
            message = "On successful getting an empty ad hoc message"),
        @ApiResponse(
            code = HttpURLConnection.HTTP_INTERNAL_ERROR,
            message = "Failure getting an empty ad hoc message"),
        @ApiResponse(
            code = HttpURLConnection.HTTP_BAD_REQUEST,
            message = "Error in getting an empty ad hoc message")
      })
  ...
  @ApiResponses(
      value = {
        @ApiResponse(
            code = HttpURLConnection.HTTP_OK,
            message = "On successful sending ad hoc message"),
        @ApiResponse(
            code = HttpURLConnection.HTTP_INTERNAL_ERROR,
            message = "Failure sending ad hoc message"),
        @ApiResponse(
            code = HttpURLConnection.HTTP_BAD_REQUEST,
            message = "Error sending ad hoc message")
      })
  @RequestMapping(method = RequestMethod.POST)
  public ModelAndView onPost(
      @ApiParam(name = "servletRequest", value = "Servlet request")
  ...
  private ModelAndView onSendAction(
    ...
    final AdHocMessageSummary summary =
        adHocMessageService.scheduleAdHocMessage(
            ...
            model.getMessageRequest().getChannels(),
            AdHocMessageChannelConfigurationBuilderFactory.builderFor(
                    Context.getAdministrationService(), model)
                .build(),
            recipients);
