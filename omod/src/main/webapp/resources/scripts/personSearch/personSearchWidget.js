/**
 * It is based on openmrs-module-coreapps/omod/src/main/webapp/resources/scripts/patientsearch/patientSearchWidget.js.
 */
function PersonSearchWidget(configuration){
    ...
    var requestCount = 0;
    var searchUrl = '/' + OPENMRS_CONTEXT_PATH + '/ws/cfl/people';
    var initialData = [];
    ...
    var searchOnIdentifiers = function(identifiers) {
        ...
        jq.each(identifiers, function (idx, identifier) {
            var deferred = emr.getJSON(searchUrl, {query: identifier})
                .then(function (data) {
    ...
    var searchOnExactIdentifierMatchThenIdentifierAndName = function(query, currRequestCount, autoSelectIfExactIdentifierMatch) {
        emr.getJSON(searchUrl, {query: query })
            .done(function (data) {
    ...
    var updateSearchResults = function(results) {
        ...
        if(results){
            ...
            _.each(results, function(person) {
                ...
                if (person.birthdate) {
                    birthdate = moment(person.birthdate).format(configuration.dateFormat);
                ...
                jq.each(attributeTypes, function(index, typeName) {
                    var attributeValue = "";
                    jq.each(person.attributes, function(index, attribute) {
                        if (typeName === attribute.name) {
