...
public class AdHocMessageServiceImplTest extends BaseModuleContextSensitiveTest {
    ...
    public void beforeTest() throws Exception {
        ...
        Context
                ...
                .setGlobalProperty(CFLConstants.COUNTRY_SETTINGS_MAP_KEY, CFLConstants.COUNTRY_SETTINGS_MAP_DEFAULT_VALUE);
        Context.getAdministrationService().setGlobalProperty(ConfigConstants.DEFAULT_USER_TIMEZONE, "UTC");
    }
    ...
    public void scheduleAdHocMessage_shouldScheduleForGivenDateTimeAndAllPatients() {
        ...
        final Set<Patient> patients =
                new HashSet<Patient>(patientAdvancedDao.getPatients(0, -1, QueryCriteria.fromConditions(Patient.class)));

        service.scheduleAdHocMessage(now, Collections.singleton(channelType),
                singletonMap(CALL_CHANNEL_CALL_FLOW, callFlowName), patients);
        ...
        final ArgumentCaptor<TaskDefinition> taskDefinitionArgumentCaptor = ArgumentCaptor.forClass(TaskDefinition.class);
        verify(schedulerServiceMock, new AtLeast(3)).saveTaskDefinition(taskDefinitionArgumentCaptor.capture());

        final Map<String, TaskDefinition> latestTaskDefinitionState = new HashMap<String, TaskDefinition>();
