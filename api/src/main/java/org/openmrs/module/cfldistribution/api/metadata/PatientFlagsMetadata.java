...
public class PatientFlagsMetadata extends VersionedMetadataBundle {
    ...
    private void addFlagPriority() {
        cflPriority = new Priority();
        cflPriority.setName("CFL flag priority");
        cflPriority.setStyle("border: 1px solid #51a351; color: #51a351; padding: 1px 2px; border-radius: 4px;");
        cflPriority.setRank(1);
        cflPriority.setUuid("15d23b9b-1dc1-448c-81ce-8c5d191b0fff");
        flagService.savePriority(cflPriority);
    }

    private void addFlags() {
        addSQLFlag("CFL HIV encounter", "Last CFL HIV encounter on ${1}",
        ...
        addSQLFlag("CFL flag - missed last 3 visits", "Missed last 3 visits",
                "SELECT\n" + "\ti.patient_id\n" + "FROM\n" + "\t(\n" + "\tSELECT\n" + "\t\tst.patient_id,\n" +
                        ...
                        "\t\tst.patient_id,\n" + "\t\tst.status\n" + "                            ) i\n" + "WHERE\n" +
                        "\ti.status = \"MISSED\"\n" + "\tAND missed_count > 2;");
        addSQLFlag("Consent Pending", "Consent Pending",
                "SELECT\n" + "\ta.patient_id\n" + "FROM\n" + "\tpatient a\n" + "WHERE\n" + "\ta.patient_id in (\n" +
                        "\tSELECT\n" + "\t\tperson_id\n" + "\tFROM\n" + "\t\tperson_attribute\n" + "\tWHERE\n" +
        ...
        addSQLFlag("Decreasing adherence", "Decreasing adherence of a patient",
                "SELECT\n" + "\tadhTrend.patient_id\n" + "FROM\n" + "\t(\n" + "\tSELECT\n" + "\t\tadhLev1.patient_id,\n" +
        ...
        addSQLFlag("Incorrect pincode", "Incorrect pincode entered last 3 times",
