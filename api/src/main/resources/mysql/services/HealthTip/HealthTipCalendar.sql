...
FROM (
         ...
         FROM (
                  SELECT *
                  FROM (SELECT * FROM DATES_LIST_10K_DAYS_TABLE) dates_list
    ...
    FROM (
          (
              ...
              WHERE
                  ...
                  WHERE :Categories_of_the_message LIKE concat('%'
                  , cn.name
                  , '%')
                  )
                AND concept_id not IN (
                  ...
                  , source_id
                  , answered_time FROM messages_actor_response mar WHERE source_type like 'SCHEDULED_SERVICE_GROUP') mar
